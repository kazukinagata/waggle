...
GUIDANCE="Waggle task protocol active. Waggle Task pages live in ${LOCATOR}. Do NOT create or modify Waggle Task pages by calling Notion MCP write tools directly (notion-create-pages / notion-update-page / notion-update-relation) — route every task create / update / status change / delegation / execution through the Waggle skills (managing-tasks, planning-tasks, executing-tasks, delegating-tasks, ingesting-messages, running-daily-tasks). Before moving any task into a Ready+ status (Ready / In Progress / In Review / Done), a live reviewing-quality run must produce a fresh Quality Verdict, and that verdict must be written in the same update as the Status change. Never promote a task to Ready+ with a bare Status-only write — promoting without a fresh review silently skips the quality review (AC/EP rubric + reproducibility check). Direct MCP writes also bypass executor-field invariants, Acknowledged At auto-set, and subtask cascading. Reading via notion-fetch / notion-search is fine."
