...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/turso-exec.sh \
...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/init-db.sh
```
...
**Ready tasks:**
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/query-tasks.sh "t.status = 'Ready'"
```

**Tasks by executor and status (single executor):**
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/query-tasks.sh "t.status = 'Ready' AND t.executor = 'cowork'"
```

**Tasks by executor and status (multiple executors — for cli/claude-desktop environments):**
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/query-tasks.sh "t.status = 'Ready' AND t.executor IN ('cli','claude-desktop','cowork')"
```

**Tasks assigned to current user:**
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/query-tasks.sh "t.assignee LIKE '%<user_id>%'"
```
...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/query-tasks.sh "t.status = 'In Progress' AND t.assignee LIKE '%<user_id>%'"
```
...
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/query-tasks.sh "t.parent_task_id = '<parent_task_id>'"
```

**Check if a task has children:**
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/query-tasks.sh "t.parent_task_id = '<task_id>'" | jq '.results | length'
```

**Check if a candidate parent is itself a subtask:**
```bash
bash ${CLAUDE_SKILL_DIR}/scripts/query-tasks.sh "t.id = '<candidate_parent_id>'" | jq '.results[0].parent_task_id'
```
...
- **Update instruction**: "Run: `bash ${CLAUDE_SKILL_DIR}/scripts/turso-exec.sh \"UPDATE tasks SET agent_output = '<result>', status = 'Done', updated_at = strftime('%Y-%m-%dT%H:%M:%SZ', 'now') WHERE id = '<task_id>';\"`"

## On Completion Template

The following template is injected into dispatch prompts by `executing-tasks`. Placeholders are resolved at dispatch time. `<absolute_path_to_turso_exec_sh>` is resolved to the absolute path of `${CLAUDE_SKILL_DIR}/scripts/turso-exec.sh` at dispatch generation time.
...
```bash
TASKS_JSON=$(bash ${CLAUDE_SKILL_DIR}/scripts/query-tasks.sh | jq -c '{tasks: [.results[] | {
...
1. Query: `bash ${CLAUDE_SKILL_DIR}/scripts/turso-exec.sh "SELECT * FROM teams;"`
