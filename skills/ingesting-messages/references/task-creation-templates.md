...
| Context | `Received via {tool_name} on {date}`. When Phase A.5 returned a non-PASS verdict for this draft, append the findings block returned by `reviewing-quality` after this line (the block carries its own delimiters; see the Ready+ rule below). |
